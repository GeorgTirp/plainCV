...
python data/datasets/prepare.py \
  --out_path=./data/datasets/outputs/fwedu/fwedu_sample_10B_tokenizer_GPT2 \
  --cache_path=./data/datasets/hf_cache \
...
DEFAULT_OUT_PATH = REPO_ROOT / "data" / "datasets" / "outputs"
DEFAULT_CACHE_PATH = REPO_ROOT / "data" / "datasets" / "hf_cache"
...
flags.DEFINE_string("out_path", str(DEFAULT_OUT_PATH), "Path where to save the dataset.")
flags.DEFINE_string("cache_path", str(DEFAULT_CACHE_PATH), "Cache for download.")
...
def main(_):
    ...
    out_path = os.path.expanduser(FLAGS.out_path)
    cache_path = os.path.expanduser(FLAGS.cache_path)
    ...
    if FLAGS.chunk:
        ...
        # Cast to numpy arrays (JAX-friendly).
        chunked_ds.set_format("numpy")
