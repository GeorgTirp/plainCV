...
try:
    import torch
except Exception:  # torch is optional for JAX-first workflows
    torch = None


def intra_doc_causal_mask(doc_boundaries: list, max_seq_length: int, device: str = "cpu"):
    """Create a block diagonal causal mask for the intra-document segments.

    Returns a torch.Tensor when torch is available, otherwise a numpy array.
    """
    ...
    # Numpy fallback to avoid torch dependency in JAX-first environments.
    sub_masks_bool = []
    for segment_length in doc_boundaries:
        segment_causal_mask_bool = np.tril(np.ones((segment_length, segment_length), dtype=bool))
        sub_masks_bool.append(segment_causal_mask_bool)

    return _block_diag_bool(sub_masks_bool)


def _block_diag_bool(sub_masks: List[np.ndarray]) -> np.ndarray:
    total = sum(mask.shape[0] for mask in sub_masks)
    block = np.zeros((total, total), dtype=bool)
    offset = 0
    for mask in sub_masks:
        length = mask.shape[0]
        block[offset : offset + length, offset : offset + length] = mask
        offset += length
    return block
